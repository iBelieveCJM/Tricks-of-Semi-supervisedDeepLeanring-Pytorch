...
class Trainer:
    ...
    def train_iteration(self, label_loader, unlab_loader, print_freq):
        ...
        for (label_x, label_y), (unlab_x, unlab_y) in zip(cycle(label_loader), unlab_loader):
            ...
            with torch.no_grad():
                ema_outputs_u = self.ema_model(unlab_x)

            ## mixup-consistency loss
            # mixup-loss-v1
            #mixed_ux, mixed_uy, lam = mixup_one_target(unlab_x, ema_outputs_u,
            #                                          self.alpha, self.device)
            #mixed_outputs_u = self.model(mixed_ux)
            ##mix_loss = softmax_loss_mean(mixed_outputs_u, mixed_uy)
            #mix_loss = mse_with_softmax(mixed_outputs_u, mixed_uy)

            # mixup-loss-v2
            mixed_ux, uy_a, uy_b, lam = mixup_two_targets(unlab_x, ema_outputs_u, 
            ...
            loss += mix_loss; loop_info['uMix'].append(mix_loss.item())
            ...
            loop_info['u2acc'].append(unlab_y.eq(ema_outputs_u.max(1)[1]).float().sum().item())
