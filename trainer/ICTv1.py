...
class Trainer:
    ...
    def train_iteration(self, data_loader, print_freq):
        ...
        for batch_idx, ((x1, x2), targets) in enumerate(data_loader):
            ...
            with torch.no_grad():
                ema_outputs = self.ema_model(x2) # x2 seems better
                ema_outputs = ema_outputs.detach()
            ## mixup consistency loss
            # mixup-loss-v1
            #mixed_x, mixed_y, lam = mixup_one_target(x1, ema_outputs, self.alpha, self.device)
            #mixed_outputs = self.model(mixed_x)
            ##mix_loss = softmax_loss_mean(mixed_outputs, mixed_y)
            #mix_loss = mse_with_softmax(mixed_outputs, mixed_y)

            # mixup-loss-v2
            mixed_x, y_a, y_b, lam = mixup_two_targets(x1, ema_outputs, self.alpha,
                                                       self.device, is_bias=False)
            mixed_outputs = self.model(mixed_x)
            ...
            loop_info['lacc'].append(targets[lmask].eq(outputs[lmask].max(1)[1]).float().sum().item())
            loop_info['uacc'].append(targets[umask].eq(outputs[umask].max(1)[1]).float().sum().item())
            loop_info['u2acc'].append(targets[umask].eq(ema_outputs[umask].max(1)[1]).float().sum().item())
